...
class GameSession {
  ...
  final GuessValidator _validator;

  GameSession({GuessEvaluator evaluator = const GuessEvaluator(), required GuessValidator validator})
  ...
  GuessOutcome submitGuess(Round round, String word) {
    ...
    final result = won ? RoundResult.won : (outOfAttempts ? RoundResult.lost : RoundResult.inProgress);

    final updated = round.copyWith(guesses: guesses, result: result, completedAt: result != RoundResult.inProgress ? DateTime.now() : null);
