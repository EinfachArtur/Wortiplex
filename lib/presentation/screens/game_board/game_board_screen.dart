...
class _GameBoardScreenState extends ConsumerState<GameBoardScreen> with WidgetsBindingObserver {
  ...
  Widget _buildBoard(BuildContext context, Round round) {
    ...
    return KeyboardListener(
      ...
      child: Column(
        children: [
          Expanded(child: _isDateGuess ? _buildDateBoard(round) : _buildLetterBoard(round)),
          VirtualKeyboard(
  ...
  /// Classic/daily/fever: the board is squarish (5-6 narrow columns), so it
  /// scales down uniformly to fit whichever dimension is tighter.
  Widget _buildLetterBoard(Round round) {
    return Center(
      child: FittedBox(
